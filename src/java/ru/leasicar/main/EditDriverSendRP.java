    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(ac.isLogIn(request.getSession().getId()) && ac.checkPermission(ac.getUserId(request.getSession().getId()), "editdriver")){
                ...
                wsql.getEditDataDriver(driverId, request.getParameter("limit"), request.getParameter("carId"),  request.getParameter("name"), 
                        request.getParameter("lastname"), request.getParameter("phone"), request.getParameter("dayRent"), 
                        request.getParameter("schedule"),  request.getParameter("driverComment"),  request.getParameter("yaId")); //dayRent
                int changedSchedue = Integer.parseInt(request.getParameter("changetSchedule"));
                if(changedSchedue==1){
                   wsql.changeStartDate(driverId); 
                }
            }
            else{
