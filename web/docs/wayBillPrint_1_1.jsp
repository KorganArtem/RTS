...
<%
    ...
    boolean showOutTime = false; //parsql.getBoolParam("showOutTime");
    WayBillSQL wbsql = new WayBillSQL();
    Map<Integer, Map> wbilList = wbsql.getWayBillTabel(request.getParameter("startDate"), request.getParameter("endDate"), cmpanyId, 0, true);
    System.out.println(wbilList.size());
    for(Entry<Integer, Map> wayBill : wbilList.entrySet())
    //for(long i=startDate.getTime(); i<=endDate.getTime(); i=i+(60*60*24*1000))
    {	
	try{
	Map row = wayBill.getValue();
	DriverSQL dsql = new DriverSQL();
	driverData = dsql.getAllDataDriver(Integer.parseInt(row.get("driverId").toString()));
	System.out.println(row.get("driverId").toString());
	...
	Date startDate1 = format.parse(row.get("waybillsDate").toString());
	carData = csql.getCarDataForAct(Integer.parseInt(row.get("carId").toString()));
        ...
        String docNum = row.get("docNumInBill").toString()+row.get("wayBillsId").toString();
        String date1 = formatOut.format(startDate1);
        String date2 = formatOut.format(startDate1.getTime()+(60*60*24*1000));
    %>
